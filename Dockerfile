...
RUN apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y software-properties-common postgresql-12 postgresql-client-12 postgresql-contrib-12
...
RUN    /etc/init.d/postgresql start &&\
    psql --command "CREATE USER my_username WITH SUPERUSER PASSWORD 'my_pass';" &&\
    createdb -O my_username my_dbname
